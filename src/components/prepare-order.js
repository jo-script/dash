...
function NewOrder() {
  ...
  async function deliverOrder(cartid) {
    if (secureLocalStorage.getItem("_tocken") !== null) {
      ...
      let result = await fetch(
        "/api/auth/supplier/orders.php?delivered=" + cartid,
        {
          method: "POST",
          body: jsonString,
          headers: {
      ...
      {orders.map((order) => (
        ...
        >
          <div className="max-[750px]:min-w-[100px] flex flex-col items-start justify-start gap-2 text-[#364e64]">
            <Link
              href={`/order/prepare-order/${order.cartid}`}
            ...
            <button
              onClick={async () => {
                await deliverOrder(order.cartid);
                router.push(`/order/delivered/${order.cartid}`);
              }}
              className="flex items-center gap-2 px-2 pb-[2px] rounded-full hover:bg-blue-400 hover:text-white transition-[all_.1s]"
            >
