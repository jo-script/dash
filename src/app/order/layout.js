      ...
      <div className=" linksOrder flex items-center justify-start gap-2 flex-wrap overflow-auto ">
        { // Create an order links by looping a variable linksOrder
          linksOrder.map((data, index) => (
            <Link key={index} href={data.link} className="min-w-[175px] text-[#f0f7ff] bg-[#224971] hover:bg-[#285480] font-medium rounded-lg text-[16px] px-5 py-2.5 text-center flex items-center flex-nowrap justify-between gap-4 active:bg-[#18334f] transition-[all_.2s] dark:focus:ring-[#3b5998]/55 max-sm:text-sm max-sm:min-w-[160px] ">
              {data.name}
              {data.icon}
            </Link>
          ))
