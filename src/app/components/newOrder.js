  ...
  return (
    <div className='w-full flex flex-col items-center gap-3 mt-10'>
      ...
      {dataOrders.map((order) => (
        <div key={order.id} className='w-[98%] h-[120px] max-[750px]:h-auto rounded-md bg-[#dae6f2] border shadow-sm  flex items-center justify-around max-[750px]:overflow-x-scroll gap-4 max-[750px]:gap-[50px] hover:scale-[1.01] transition-[all_.02s]'>
          ...
          <div className=' max-[750px]:min-w-[100px] flex flex-col items-start justify-start gap-2 text-[#364e64]'>
            <Link href={`/order/new-order/${order.id}`} className='flex items-center gap-2 px-2 pb-[2px] rounded-full hover:bg-blue-400 hover:text-white transition-[all_.1s]'>
              <span>عرض</span>
              <BiShow />
            </Link>
